...
const RC910_STRUCTURES = [
  // THE NEEDLES
  lmColumn('needles-0', 690, -18, 9.2, -5, 2.4, 19),
  lmColumn('needles-1', 690, -13.5, 6.7, 0, 2.7, 14),
  lmColumn('needles-2', 690, 17.5, 10.7, 5, 3.0, 22),

  // BIG DROP — side blocks, overhead bar and hanging pennant.
  lmBox('big-drop-left', 1240, -18.8, 5.1, 2, 2.5, 12, 11),
  lmBox('big-drop-right', 1240, 18.8, 5.1, 2, 2.5, 12, 11),
  lmBox('big-drop-arch', 1240, 0, 10.7, 1, 35, 1.1, 1.1),
  lmColumn('big-drop-pennant', 1240, 0, 8.9, 1, 1.6, 3.5),

  // SKY RAMP — support hardware is solid; the snow wing remains a rideable surface.
  lmBox('sky-ramp-tower-l', 4680, -17, 6.5, 2, 2.1, 15, 2.1),
  lmBox('sky-ramp-tower-r', 4680, 17, 6.5, 2, 2.1, 15, 2.1),
  lmBox('sky-ramp-beam', 4680, 0, 13.3, 2, 35, 1.1, 2.2),
  ...
  ...Array.from({ length: 5 }, (_, n) => {
    ...
    const x = i % 2 ? -15 : 15;
    const z = i * 12;
    const chairX = x + (i % 2 ? 3 : -3);
    return [
      lmColumn(`dead-lift-pole-${n}`, 6480, x, 5.8, z, 0.65, 13),
      lmBox(`dead-lift-bar-${n}`, 6480, x, 11.6, z, 7.5, 0.5, 0.5),
      lmBox(`dead-lift-chair-${n}`, 6480, chairX, 8.0, z + 3, 2.5, 0.35, 1.1),
    ];
  }).flat(),

  // RIBCAGE — legs plus coarse high-arch volumes; the centre remains open.
  ...Array.from({ length: 10 }, (_, i) => {
    const z = (i - 4.5) * 7;
    return [
      lmColumn(`ribcage-${i}-l`, 7480, -19.5, 6.5, z, 1.35, 18),
      lmColumn(`ribcage-${i}-r`, 7480, 19.5, 6.5, z, 1.35, 18),
      lmColumn(`ribcage-${i}-arch-l`, 7480, -11.5, 15.6, z, 2.0, 4.4),
      lmColumn(`ribcage-${i}-arch-c`, 7480, 0, 19.1, z, 2.3, 4.0),
      lmColumn(`ribcage-${i}-arch-r`, 7480, 11.5, 15.6, z, 2.0, 4.4),
    ];
  }).flat(),

  // WHITE GATE
  lmBox('white-gate-left', 8620, -14.5, 8.8, 0, 3.2, 20, 3.2),
  lmBox('white-gate-right', 8620, 14.5, 8.8, 0, 3.2, 20, 3.2),
  lmBox('white-gate-top', 8620, 0, 17.4, 0, 31, 2.0, 3.2),

  // THE WALL — central gap remains honest, including the visible marker teeth.
  lmBox('wall-left', 9760, -13.5, 10.2, 0, 17, 24, 5),
  lmBox('wall-right', 9760, 13.5, 10.2, 0, 17, 24, 5),
  lmColumn('wall-marker-l', 9760, -4.3, 5.0, -3, 1.1, 6),
  lmColumn('wall-marker-r', 9760, 4.3, 5.0, -3, 1.1, 6),
  ...
  ...Array.from({ length: 12 }, (_, i) => {
    ...
    const r = 1.3 + (i % 3) * 0.4;
    const z = (i - 5.5) * 4.5;
    return lmColumn(`black-glass-${i}`, 11020, x, h * 0.5 - 0.3, z, r, h);
  }),

  // LAST LIFT + its fallen mast.
  ...Array.from({ length: 5 }, (_, n) => {
    const i = n - 2;
    const x = i % 2 ? -15 : 15;
    const z = i * 12;
    const chairX = x + (i % 2 ? 3 : -3);
    return [
      lmColumn(`last-lift-pole-${n}`, 12420, x, 5.8, z, 0.65, 13),
      lmBox(`last-lift-bar-${n}`, 12420, x, 11.6, z, 7.5, 0.5, 0.5),
      lmBox(`last-lift-chair-${n}`, 12420, chairX, 8.0, z + 3, 2.5, 0.35, 1.1),
    ];
  }).flat(),
  lmBox('last-lift-fallen', 12420, 0, 3.0, 2, 21.5, 7.0, 1.6),

  // AFTERLIGHT's ring is intentionally horizontal in the current art, with
  // upright side pylons. Collision matches that presentation rather than
  // pretending it is a vertical hoop.
  lmFlatRing('afterlight-ring', 14100, 0, 11.5, 0, 15.5, 1.25),
  lmColumn('afterlight-pillar-l', 14100, -17.5, 7.0, 0, 1.2, 18),
  lmColumn('afterlight-pillar-r', 14100, 17.5, 7.0, 0, 1.2, 18),

  // BELL TOWER
  lmBox('bell-tower-left', 15880, -15, 9, 0, 2.2, 20, 2.2),
  lmBox('bell-tower-right', 15880, 15, 9, 0, 2.2, 20, 2.2),
  lmBox('bell-tower-top', 15880, 0, 18.0, 0, 32, 1.5, 2.4),
  lmColumn('bell-tower-bell', 15880, 0, 13.7, 0, 4.0, 4.8),
  lmColumn('bell-tower-clapper', 15880, 0.2, 10.8, 0, 0.7, 1.4),

  // THE MOUTH — jaws are ground-up, teeth only threaten large air.
  lmBox('mouth-left', 17760, -16, 7.2, 0, 9.5, 18, 8.5),
  lmBox('mouth-right', 17760, 16, 7.2, 0, 9.5, 18, 8.5),
  ...Array.from({ length: 9 }, (_, n) => {
    const i = n - 4;
    return lmColumn(`mouth-tooth-${n}`, 17760, i * 3.3, 12.5, -1, 1.3, 7.5);
  }),

  // MOONSHOT support hardware + the high horizontal moon ring. The snow wing
  // stays rideable and is represented by the authored terrain launch instead.
  lmBox('moonshot-tower-l', 21820, -21.25, 8.775, 2.36, 2.625, 20.25, 2.48),
  lmBox('moonshot-tower-r', 21820, 21.25, 8.775, 2.36, 2.625, 20.25, 2.48),
  lmBox('moonshot-beam', 21820, 0, 17.955, 2.36, 43.75, 1.49, 2.60),
  lmFlatRing('moonshot-ring', 21820, 0, 27.0, -2.36, 6.2, 0.95),

  // Distance-marker boards/poles are scenery you can actually crash into.
  ...[5000, 10000, 15000, 20000, 25000, 30000].flatMap((d) => [
    lmBox(`distance-${d}-board`, d, -13, 5.7, 0, 7.2, 2.7, 0.5),
    lmBox(`distance-${d}-pole-l`, d, -16, 2.4, 0, 0.32, 6.8, 0.32),
    lmBox(`distance-${d}-pole-r`, d, -10, 2.4, 0, 0.32, 6.8, 0.32),
  ]),
];
