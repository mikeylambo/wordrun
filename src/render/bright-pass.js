...
export const BLOOM = {
  THRESHOLD: 0.30,   // luminance where a pixel starts to bleed
  KNEE: 0.30,        // soft shoulder above it, so nothing pops on
  STRENGTH: 0.55,    // how much of the blur is added back
...
export class BrightPass {
  constructor(renderer) {
    ...
    this.blurU = {
      ...
      uRadius: { value: BLOOM.RADIUS_TIGHT },
    };
    this.compU = {
      tColor: { value: this.scene.texture },
  ...
  render(renderer, scene, camera, reducedFlash) {
    ...
    for (const r of [BLOOM.RADIUS_TIGHT, BLOOM.RADIUS_WIDE]) {
