...
const iconSource = read('public/icons/wordrun.svg');
const pngPaths = [
  'public/apple-touch-icon.png',
  'public/icons/wordrun-180.png',
  'public/icons/wordrun-192.png',
  'public/icons/wordrun-512.png',
  'public/icons/wordrun-maskable-512.png',
];
...
check(manifest.name === 'WORD RUN' && manifest.short_name === 'WORDRUN',
  'PWA has canonical WORD RUN app identity');
...
check(Array.isArray(manifest.icons) &&
      manifest.icons.some((i) => i.src === './icons/wordrun-192.png' && i.type === 'image/png') &&
      manifest.icons.some((i) => i.src === './icons/wordrun-512.png' && i.type === 'image/png'),
  'PWA manifest exposes explicit PNG 192 and 512 install icons');
check(manifest.icons.some((i) => i.src === './icons/wordrun-maskable-512.png' && String(i.purpose || '').includes('maskable')),
...
check(iconSource.includes('#ff2a1f') && iconSource.includes('trail'),
  'spoiler-safe source carries the red descent-trail identity');
check(index.includes('rel="manifest" href="./manifest.webmanifest?v=3"') &&
      index.includes('rel="apple-touch-icon" sizes="180x180" href="./apple-touch-icon.png?v=3"') &&
...
check(sw.includes("const CACHE = 'wordrun-v1-shell-1'") &&
      sw.includes("new URL('apple-touch-icon.png', BASE).href") &&
      sw.includes("new URL('icons/wordrun-512.png', BASE).href") &&
